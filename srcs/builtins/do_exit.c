...
/*   Updated: 2023/02/01 23:36:10 by oboutarf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int exit_numeric_char(t_mshell *mshell)
{
    ...
    i = 0;
    while (mshell->exec->start_exec->next && mshell->exec->start_exec->type != _ARG)
        mshell->exec->start_exec = mshell->exec->start_exec->next;
    while (mshell->exec->start_exec->tkn[i])
    {
        if (!(mshell->exec->start_exec->tkn[i] >= '0' && mshell->exec->start_exec->tkn[i] <= '9'))
            return (0);
        i++;
    }
    mshell->exec->start_exec = mshell->exec->start_exec_head;
